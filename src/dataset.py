...
class Dataset:
	...
	def _load_retinopathy256(self):
		# Big dataset
		self._big_dataset = True

		# Load dataframes
		self._df_trainval = pd.read_csv(os.path.join(DATASETS_DIR, 'retinopathy/data256/train.csv'))
		self._df_test = pd.read_csv(os.path.join(DATASETS_DIR, 'retinopathy/data256/test.csv'))
		...
		# Set base path for images
		self._base_path = os.path.join(DATASETS_DIR, 'retinopathy/data256/')
	...
	def generate_train(self, batch_size, augmentation, labels_transform=lambda labels, n: to_categorical(labels, num_classes=n)):
		# Load dataset if not loaded
		self.load(self._name)

		if self._big_dataset:
			return BigGenerator(self._df_train, self._base_path, self._num_classes, self._x_col, self._y_col, mean=self.mean_train, std=self.std_train, batch_size=batch_size, augmentation=augmentation, labels_transform=labels_transform)
		else:
			return SmallGenerator(self._x_train, self._y_train, self._num_classes, mean=self.mean_train, std=self.std_train, batch_size=batch_size, augmentation=augmentation, labels_transform=labels_transform)

	def generate_val(self, batch_size, labels_transform=lambda labels, n: to_categorical(labels, num_classes=n)):
		# Load dataset if not loaded
		self.load(self._name)

		if self._big_dataset:
			return BigGenerator(self._df_val, self._base_path, self._num_classes, self._x_col, self._y_col, mean=self.mean_train, std=self.std_train, batch_size=batch_size, labels_transform=labels_transform)
		else:
			return SmallGenerator(self._x_val, self._y_val, self._num_classes, mean=self.mean_train, std=self.std_train, batch_size=batch_size, labels_transform=labels_transform)
